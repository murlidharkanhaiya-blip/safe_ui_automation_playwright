...
class FilterChipLiveStreaming {
    constructor(page) {
        this.page = page;

        // Improved Locators
        this.liveStreamingCard = page.locator("div.dashboard-box").nth(4); // 0-indexed, so [5] = nth(4)
        this.filterChip = page.locator("div.filter-icon.hand").first();
        this.channelDropdown = page.locator("div.inputBoxDiv.ellipsis").first(); // First dropdown for Channel
    ...
    async selectAndApplyFilter(filterName) {
        ...
        if (!filterSelected) {
            ...
            const allText = await this.page.locator('text=Windows, text=Android, text=Chromebook, text=Macos').all();
    ...
    async verifylivestreamingfilterchip() {
        // ✅ Reset state
        await this.page.waitForLoadState('domcontentloaded');
        await this.page.waitForTimeout(2000);
        ...
        // ✅ Step 1: Click Live Streaming card
        console.log("📊 Looking for Live Streaming card...");
        
        await this.liveStreamingCard.waitFor({ state: 'visible', timeout: 15000 });
        await this.liveStreamingCard.scrollIntoViewIfNeeded();
        await this.page.waitForTimeout(500);
        await this.liveStreamingCard.click();
        console.log("✅ Clicked Live Streaming card");
        ...
        if (noDataVisible) {
            console.warn("⚠️ No data available on Live Streaming");
            return;
        }
        ...
        // ✅ Step 2: Apply filters (exact names from screenshot)
...
module.exports = { FilterChipLiveStreaming };