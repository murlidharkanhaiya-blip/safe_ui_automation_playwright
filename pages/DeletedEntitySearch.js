...
class DeletedEntitySearch {
    ...
    async waitForDeletedEntitiesToLoad() {
        await this.deletedEntityCard.waitFor({ state: 'visible', timeout: 20000 });

        const cardHandle = await this.deletedEntityCard.elementHandle();

        await this.page.waitForFunction(
            (el) => {
                const text = el?.innerText?.trim() || '';
                const match = text.match(/Deleted Entities\s*(\d+)/);
    ...
    async verifySearchondeletedentityPage() {
        await this.page.waitForLoadState('domcontentloaded');
        await this.page.waitForTimeout(1000);

        await this.waitForDeletedEntitiesToLoad();
        await this.deletedEntityCard.scrollIntoViewIfNeeded();
        await this.deletedEntityCard.waitFor({ state: 'visible', timeout: 5000 });
    ...
    async performSearchAndAssert(criteriaText, inputValue, cellSelector, label) {
        ...
        // ⏳ Retry up to 3 times within 10s
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                await this.page.waitForTimeout(1500); // small delay
...
module.exports = { DeletedEntitySearch };
